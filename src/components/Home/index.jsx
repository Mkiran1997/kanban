...
const Dashboard = () => {
  ...
  const handleChange = useDebounce((searchPatient, setState) => {
  ...
  const fetchColumn = async () => {
    try {
      const data = await GET("/board/1/columns");
  ...
  const fetchCategory = async () => {
    try {
      const data = await GET("/tasks/categories");
            ...
            <div className="flex flex-col lg:flex-row lg:justify-between items-center px-3 py-2 gap-3">
              <div className="relative py-1 max-w-full lg:max-w-[300px] w-full">
                ...
                <input
                  ...
                  name="searchParam"
                  onChange={(e) => {
                    handleChange(e.target.value, setSearchParam);
                  }}
                />
              </div>
